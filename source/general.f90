...
! Minimum interatomic distance (distance H-H in H2 = 0.7414 A)
  Dist_atom_min = 0.7_db / bohr
 ...
 !           if( iabsfirst <= n_atom_per + n_atom_int .and. iz < 0 ) cycle
          ...
          if( .not. ATA .and. .not. Abs_case ) then
            do jgr = igr0,igr-1
              ...
              if( sum( abs( posg(:,igr) - posg(:,jgr) ) ) < Dist_atom_min )  cycle boucle_igr
            end do
          endif
      ...
      do iy = -nymaille,nymaille
        ...
        do iz = 0,-nzmaille,-1
          ...
          boucle_igr_bulk: do igr = 1,n_atom_bulk
            ...
            if( .not. ATA ) then
              do jgr = igr0,igr-1
               if( sum( abs( posg_bulk(:,igr) - posg_bulk(:,jgr) ) ) < Dist_atom_min )  cycle boucle_igr_bulk
              end do
            endif
...
subroutine Clust(angxyz,angxyz_bulk,angxyz_int,angxyz_sur,ATA,axyz,axyz_bulk,axyz_int,axyz_sur,Base_ortho,Base_ortho_int, &
  ...
  integer:: ia, ia1, ia2, iaabs, iaabsfirst, iabsorbeur, iabsfirst, ib, igr, igr_b, igr0, igr_dop, igr12, ipr, iprint, &
...
! directions cristallographiques.
  ...
  130 format('     Atom',i4,', nbordf =',i5,', nbord =',i5)
