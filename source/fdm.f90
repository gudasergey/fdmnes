...
subroutine fdm(Analyzer,Ang_borm,Bormann,Check_extract,Check_mpi,Circular,Comt,Convolution_cal,Delta_edge,E_cut_imp, &
  ...
  integer:: i, igr, igr_dop, iord, ipr, ipr_dop, ir, is, iscratch, istat, it, itape1, itape4, itph, itpm, itps, &
    itype_dop, j, jgr, jseuil, l, lecrantage, Length_line, Lin_gam, Lmax_DOSout, Lmax_nrixs, Lmax_pot, Lmax_pot_c, &
    ...
    n_atom_sur, n_atom_uc, n_devide, n_file_dafs_exp, n_mat_polar, n_multi_run, n_multi_run_e, &
    natomsym_max, n_adimp, n_bulk_sup, n_q_rixs, n_radius, n_range, n_shift_core, n_theta_in, n_theta_rixs, n_two_theta, &
    n_Z_abs, nb_atom_conf_m, nbseuil, ncolm, necrantage, neimagent, nenerg_loss_rixs, nenerg_out_rixs, nenerg_in_rixs, &
    nenerg_s, neqm, ngamh, ngamme, ngroup, ngroup_hubb, ngroup_lapw, ngroup_m, ngroup_nonsph, ngroup_par, ngroup_pdb, &
    ...
    nslapwm, nspin, nspino, nspinp, ntype, ntype_bulk, ntype_conf, ntype_mod, Test_reg, Trace_k, Wien_save, Z_nospinorbite
  ...
  logical:: Absauto, Abs_in_bulk, All_nrixs, All_site_rixs, Allsite, Ampl_rixs, Analyzer, ATA, Atom_nonsph, Atom_occ_hubb, &
     Atomic_scr, Axe_loc, Basereel, Base_ortho_int, Base_ortho_sur, Bormann, Bulk, &
     Check_extract, Check_mpi, Clementi, Cap_layer, Cartesian_tensor, Center_s, Charge_free, Circular, Cluster_Approach, &
     Convolution_cal, COOP, Coop_z_along_bond, Core_energ_tot, Core_resolved, Coupelapw, Dafs, &
     ...
     Green_bulk, Green_int, Green_s, Green_self, Harm_cubic, Helm_cos, Helm_lin, Helm_mix, hkl_film, Hubbard, Kern_fast, key_calc, &
     korigimp, lmaxfree, lmoins1, lplus1, Magnetic, Matper, Memory_save, Moment_conservation, Muffintin, No_DFT, No_interference, &
     No_solsing, Noncentre, Nonexc, Normaltau, NRIXS, Numerov, Occupancy_first, Octupole, &
     Old_zero, One_run, One_SCF, Only_rixs, Operation_mode_used, Optic, Overad, Pdb, PointGroup_Auto, Polarise, Powder, &
     Quadmag, Quadrupole, Readfast, Relativiste, Renorm, RIXS, RIXS_core, RIXS_db_conv, RIXS_fast, RPALF, Rydberg, Scan_a, &
  ...
  real(kind=db), dimension(:), allocatable:: Adimp_e, Angle_or, cdil, E_adimp, E_max_range, E_radius, Ecrantage, &
     Eeient, Egamme, Eimagent, Energ_loss_rixs, Energ_out_rixs, Energ_rixs, r0_lapw, r0_lapw_temp, Rchimp, rchimp_temp, Rlapw, &
     rlapw_temp, rmt, rmt_temp, Rmtimp, rmtimp_temp, Rsorte_s, Taux_cap, Taux_oc, Temp_coef, V_hubbard, V_hubbard_temp, V0bdcFimp
  ...
  call lectdim(Absauto,Atom_occ_hubb,Atom_nonsph,Axe_loc,Bormann,Bulk,Cap_layer,Dafs_bio,Doping,Extract,Extract_ten,Film, &
    ...
    n_file_dafs_exp,n_mat_polar,n_multi_run_e,n_q_rixs,n_radius,n_range,n_theta_in,n_theta_rixs,n_two_theta,n_Z_abs, &
    nb_atom_conf_m,ncolm,neimagent,nenerg_in_rixs,nenerg_loss_rixs,nenerg_out_rixs,nenerg_s,ngamme, &
  ...
  allocate( Energ_loss_rixs(nenerg_loss_rixs) )
  allocate( Energ_out_rixs(nenerg_out_rixs) )
  ...
  call lecture(Absauto,adimp_e,alfpot,All_nrixs,All_site_rixs,Allsite,Ampl_rixs,Ang_borm,Ang_rotsup,Angle_mode,Angle_or, &
    ...
    axyz_cap,axyz_int,axyz_sur,Basereel,Bormann,Bulk,Bulk_roughness,Cap_B_iso,Cap_layer,Cap_roughness,Cap_shift, &
    Cap_thickness,Cartesian_tensor,cdil,Center_s,Centre,Charge_free,Check_extract,Check_mpi,Clementi,Cluster_Approach,Com,Comt, &
    COOP,Coop_z_along_bond,Core_energ_tot,Core_resolved,Coupelapw,D_max_pot,Dafs,Dafs_bio,Delta_En_conv,Delta_Epsii, &
    Delta_helm,Density,Density_comp,Dip_rel,Dipmag,Dist_coop,Doping,dpos,Dyn_eg,Dyn_g, &
    E_adimp,E_radius,E_max_range,Eclie,Eclie_out,Ecrantage,Eeient,Egamme,Eimagent,Eneg_i,Eneg_n_i, &
    Energ_loss_rixs,Energ_out_rixs,Energ_rixs,Energphot,Ephot_min,Extract,Extract_ten,f_no_res,FDM_comp,FDMX_only,Film, &
    Film_roughness,Film_shift,Film_thickness,Fit_cal,Flapw,Flapw_new,Force_ecr,Full_atom_e,Full_potential,Full_self_abs, &
    Gamma_hole,Gamma_hole_man,Gamma_max,Gamma_tddft,Green_bulk,Green_int,Green_s,Green_self,Harm_cubic,Helm_cos,Helm_lin, &
    Helm_mix,hkl_borm,hkl_dafs,hkl_film,hkl_ref,Hubb, &
    ...
    Lmax_pot,Lmax_pot_c,Lmax_tddft_inp,lmaxfree,Lmaxso_max,Lmaxso0,Lmaxat0,lmoins1,lplus1,Lseuil,lvval,m_hubb_e,Magnetic,Mat_or, &
    Mat_UB,Matper,Moment_conservation,mpinodes,mpinodes0,mpirank,mpirank0,Muffintin,Multipole, &
    ...
    nself,nself_bulk,nseuil,nslapwm,nspin,nsymextract,ntype,ntype_bulk,ntype_conf,ntype_mod,numat,numat_abs,Numerov, &
    ...
    r0_lapw,Ray_max_dirac,Rchimp,Readfast,Relativiste,Renorm,RIXS,RIXS_core,RIXS_db_conv,RIXS_fast,Rlapw,Rmt,Rmtimp,Rot_Atom_gr, &
    rotloc_lapw,roverad,RPALF,rpotmax,Rydberg,Rsorte_s,SCF_log,Self_abs, &
    ...
    Tensor_imp,Test_dist_min,Test_reg,Theta_rixs,Trace_format_wien,Trace_k,Trace_p,Typepar,Use_FDMX,V_helm,V_hubbard,V_intmax, &
...
! Determination of supplementary output files when abs in bulk with different z
  ...
  do multi_run_e = 1,n_multi_run
    ...
    if( iabsm(multi_run) <= ngroup - n_atom_bulk  .or. .not. DAFS ) cycle
...
! ------- Calculation of all the non equivalent atoms signal ----------------------------------------
  ...
  call Site_calculation(adimp_e,alfpot,All_nrixs,All_site_rixs,Allsite,Ampl_rixs,Analyzer,Ang_rotsup,Angle_mode,Angle_or, &
      Angpoldafs,Angxyz,Angxyz_bulk, &
      ...
      Cap_layer,Cap_roughness,Cap_shift,Cap_thickness,Cartesian_tensor,cdil,Center_s,Centre,Charge_free,Circular, &
      Clementi,Cluster_Approach,Com,COOP,Coop_z_along_bond,Core_energ_tot,Core_resolved,Coupelapw,D_max_pot,Dafs,Dafs_bio, &
      Delta_edge,Delta_En_conv, &
      ...
      E_radius,E_max_range,Ecent,Eclie,Eclie_out,Ecrantage,Eeient,Egamme,Eimagent,Elarg,Eneg_i,Eneg_n_i,Energ_loss_rixs, &
      Energ_out_rixs,Energ_rixs,Energphot,Ephot_min,Epsii_ref,Epsii_ref_man,Estart,Extract,Extract_ten,f_no_res, &
      ...
      Green_s,Green_self,Harm_cubic,Helm_cos,Helm_lin,Helm_mix,hkl_dafs,hkl_film,hkl_ref,Hubb,Hubbard,hybrid,iabsm,iabsorig, &
      icheck,icom,igr_coop,igr_dop,igreq,Interface_shift,ipr_dop,iscratch,isigpi,itdil,its_lapw,iord,isymqa,itype,itype_dop, &
      jseuil,Kern_fac,Kern_fast,Kgroup,korigimp,ldil,lecrantage,Lin_gam,Lmax_DOSout,Lmax_nrixs, &
      Lmax_pot,Lmax_pot_c,Lmax_tddft_inp,lmaxfree,Lmaxso_max,Lmaxso0,Lmaxat0,lmoins1,lplus1,Lseuil,lvval,m_hubb_e, &
      ...
      n_atom_sur,n_atom_uc,n_bulk_sup,n_devide,n_mat_polar,n_multi_run,n_q_rixs,n_radius,n_range,n_rout,n_shift_core,n_theta_rixs, &
      natomeq_s,natomsym_max,nbseuil,ncolm,necrantage, &
      neimagent,nenerg_in_rixs,nenerg_loss_rixs,nenerg_out_rixs,nenerg_s,neqm,ngamh,ngamme,ngreq,ngroup,ngroup_hubb,ngroup_lapw, &
      ngroup_m,ngroup_nonsph,ngroup_pdb,ngroup_taux,ngroup_temp,nhybm,nklapw,nlm_pot,nlmlapwm,nlat,nlatm,nmatsym,No_DFT, &
      No_interference,No_solsing,nom_fich_extract,nomfich,nomfich_cal_conv,nomfich_cal_tddft_conv,Noncentre, &
      ...
      nself,nself_bulk,nseuil,nslapwm,nspin,nspino,nspinp,nsymextract,ntype,numat,numat_abs(1),Numerov,nvval,occ_hubb_e, &
      ...
      pop_nonsph,popats,popval,posn,posn_bulk,posn_cap,Powder,q_nrixs,q_rixs,Quadrupole,R_rydb,R_self,R_self_bulk,r0_lapw, &
      Ray_max_dirac,Rchimp,Relativiste,Renorm,RIXS,RIXS_core,RIXS_db_conv,RIXS_fast,Rlapw,rmt,Rmtimp,Rot_Atom_gr,rotloc_lapw, &
      roverad,RPALF,rpotmax,Rydberg,Rsorte_s,SCF_log,Self_abs,Shift_core,Skip,Solsing_s,Solsing_only,Spherical_signal, &
      Spherical_tensor,Spin_channel,Spinorbite,State_all,State_all_out,Step_loss,Sum_rixs,Supermuf,Surface_shift,Symmol, &
      Taux,Taux_cap,Taux_oc,Tddft,Temp,Temp_coef,Temp_B_iso,Tensor_imp,Test_dist_min,Test_reg,Theta_rixs,Time_rout, &
      Trace_format_wien,Trace_k,Trace_p,V_helm,V_hubbard,V_intmax,Vec_orig,Vecdafsem,Vecdafssm,Veconde,V0bdcFimp,Width_helm, &
      Wien_file,Wien_matsym,Wien_save,Wien_taulap,Write_modQ,Xan_atom,Ylm_comp_inp,Z_bulk,Z_cap,Z_nospinorbite)
...
! Desallocation des tables allocated before subroutine lecture
  ...
  deallocate( Egamme,  Eeient, Eimagent, Energ_loss_rixs, Energ_out_rixs, Energ_rixs )
  ...
  deallocate( Taux_cap, Taux_oc, Temp_coef, Theta_rixs )
...
subroutine Site_calculation(adimp_e,alfpot,All_nrixs,All_site_rixs,Allsite,Ampl_rixs,Analyzer, Ang_rotsup,Angle_mode,Angle_or, &
      Angpoldafs,Angxyz,Angxyz_bulk, &
      Angxyz_cap,Angxyz_int,Angxyz_sur,ATA,Atom_occ_hubb,Atom_nonsph,Atom_with_axe,Atomic_scr,Axe_atom_gr,axyz,axyz_bulk, &
      axyz_cap,axyz_int,axyz_sur,Base_ortho_int, Base_ortho_sur,Basereel,Bormann,Bulk,Bulk_roughness,Cap_B_iso, &
      Cap_layer,Cap_roughness,Cap_shift,Cap_thickness,Cartesian_tensor,cdil,Center_s,Centre,Charge_free,Circular, &
      ...
      Nonexc,norbdil,norbv,Normaltau,normrmt,nphi_dafs,nphim, &
      npl_2d,npldafs,npldafs_2d,npldafs_e,npldafs_f,nple,nplrm,nposextract,nq_nrixs,nrato,nrato_dirac,nrato_lapw,NRIXS,nrm, &
      nself,nself_bulk,nseuil,nslapwm,nspin,nspino,nspinp,nsymextract,ntype,numat,numat_abs_1,Numerov,nvval,occ_hubb_e, &
      Octupole,Old_zero,One_run,One_SCF,Only_rixs,Operation_mode,Operation_mode_used,Optic,Overad,Overlap,p_self_max, &
      p_self0,p_self0_bulk,Pas_SCF,pdpolar,phi_0,PointGroup,PointGroup_Auto,Polar,Polarise,poldafsem,poldafssm, &
      pop_nonsph,popats,popval,posn,posn_bulk,posn_cap,Powder,q_nrixs,q_rixs,Quadrupole,R_rydb,R_self,R_self_bulk,r0_lapw, &
      Ray_max_dirac,Rchimp,Relativiste,Renorm,RIXS,RIXS_core,RIXS_db_conv,RIXS_fast,Rlapw,rmt,Rmtimp,Rot_Atom_gr,rotloc_lapw, &
  ...
  integer:: extract_nenerg, extract_nlmomax, i, i_adimp, i_cycle, i_radius, i_range, i_self, ia, iaabs, iaabsfirst, iabsfirst, &
    iaabsi, iabsorbeur, &
    ...
    l1, l2, lecrantage, lh, Lin_gam, lla_state, lla2_state, Lm1, Lm2, Lmax, Lmax_abs, Lmax_abs_a, Lmax_abs_t, Lmax_cluster, &
    Lmax_comp, Lmax_DOSout, Lmax_g, Lmax_nrixs, Lmax_pot, Lmax_pot_c, Lmax_probe, Lmax_tddft, Lmax_tddft_inp, Lmax_max, Lmaxat0, &
    Lmaxso, Lmaxso_m, Lmaxso_max, Lmaxso0, Lms1, Lms2, Lseuil, m, m_hubb, m_hubb_e, m1, m2, &
    ...
    n_atom_proto_uc, n_atom_sur, n_atom_uc, n_bulk_sup, n_bulk_z, n_bulk_z_abs, n_bulk_z_max, n_bulk_z_max_abs, &
    n_devide, n_Ec, n_mat_polar, n_max, n_multi_run, n_oo, n_orb_rel, n_orbexc, n_q_rixs, n_radius, n_range, &
    n_rel, n_RI, n_rout, n_shift_core, n_tens_max, n_theta_rixs, n_V, &
    n_vr_0, n_vr_ind, n1, nab_coop, natome, natome_cal, natome_self, natomsym_max, natomeq, natomeq_coh, natomeq_self, &
    natomp, natomsym, nb_rep, nb_sym_op, nbbseuil, nbm, nbseuil, nbtm, nbtm_fdm, ncolm, ncolr, ncolt, ndim2, necrantage, &
    neimagent, nenerg, nenerg_coh, nenerg_in_rixs, nenerg_loss_rixs, nenerg_out_rixs, nenerg_rixs, nenerg_s, nenerg_tddft, &
    nenerg0, neqm, ngamh, ngamme, nge, ngrm, ngroup, ngroup_hubb, ngroup_lapw, ngroup_m, ngroup_nonsph, ngroup_pdb, &
    ngroup_taux, ngroup_temp, ngrph, nhybm, nicm, nim, ninit1, ninit, ninit_out, ninitr, ninitlv, nklapw, nlatm, nlm, &
    nlm_comp, nlm_pot, nlm_pot_cp, nlm_pot_g, nlm_pot_r, nlm_probe, nlm_p_fp, nlm_rixs, &
    nlmagm, nlmamax, nlmlapwm, nlmmax, nlmomax, nlms_pr, nlmsam, nlmsamax, nlmso, nmatsym, nnlm, norbdil, normrmt, &
    nphiato1, nphiato7, nphim, npldafs, npldafs_2d, npldafs_e, npldafs_f, nple, nplr, nplrm, npoint, npoint_ns, npr, npso, npsom, &
    nptmoy, nptmoy_out, nq_nrixs, nr, nr_cluster, nr_core, nrato_abs, nrato_dirac, nrm, nrm_self, ns_dipmag, nself, nself_bulk, &
    nself_c, &
    nseuil, nslapwm, nso1, nsort, nsortf, nsm, nspin, nspino, nspinp, nspinorb, nstm, ntype, numat_abs, numat_abs_1, nvois, nx, &
    nxanout, Test_reg, Trace_k, Trace_l, Wien_save, Z, Z_nospinorbite
  ...
  integer, dimension(:,:), allocatable:: ia_eq, ibord, igr_bulk_z, igr_bulk_z_abs, index_coop, indice, iopsym_atom, is_eq, &
     isbord, isvois, ivois, iso, Lm_cluster, lso, m_g, mso, nb_rpr, nlmsa0
  ...
  character(len=Length_name), dimension(9):: Wien_file
  character(len=Length_name):: File_name_int
  ...
  complex(kind=db), dimension(:,:), allocatable:: Bragg_abs, Bragg_rgh_bulk_abs, karact, pol, Sum_Bragg_bulk_abs_f0, &
                                                  Sum_Bragg_nonabs_f, Vc_cluster_cp
  complex(kind=db), dimension(:,:,:), allocatable:: phdt, poldafse, poldafss, Rho_cluster_cp, Vr_cluster_cp, Vxc_cluster_cp
  ...
  complex(kind=db), dimension(:,:,:,:,:), allocatable:: Ampl_dft, Ampl_abs, S_nrixs, S_nrixs_m, secdd, secdd_m, Tau_comp, rof0, &
                                            secdq, secdq_m, Tau_ato, Tau_RI_abs, Taull, Taull_dft, Taull_tdd, V_hubb, V_hubb_s
  complex(kind=db), dimension(:,:,:,:,:,:), allocatable:: secdo, secdo_m, secoo, secoo_m, secqq, secqq_m, Rad_ioRIoi, Tau_coop, &
                                                          Tau_RI, Taull_stk
  complex(kind=db), dimension(:,:,:,:,:,:,:), allocatable:: RadialIntegral, Taull_abs

  logical:: Abs_in_bulk, Abs_in_Bulk_roughness, Absorbeur, All_nrixs, All_site_rixs, Allsite, Ampl_rixs, ATA, Analyzer, &
     Bulk_atom_done, Atom_comp_cal, Atom_nonsph, &
     Atom_nonsph_loc, Atom_occ_hubb, Atomic_scr, Basereel, Base_hexa, Base_ortho_int, Base_ortho_sur, Bormann, Bulk, Bulk_step, &
     Circular, Clementi, Cal_xanes, Cap_layer, Cartesian_tensor, Center_s, Charge_free, Cluster_Approach, &
     ...
     Extract_ten, FDM_comp, Fermi, Fermi_first, Film, Final_optic, Final_tddft, First_E, First_Bulk_step, &
     First_run, Flapw, Flapw_new, Force_ecr, Full_atom, Full_atom_e, Full_atom_gen, Full_potential, Full_self_abs, &
     Gamma_hole_man, Gamma_tddft, Green, Green_bulk, Green_i, Green_int, Green_s, Green_self, Harm_cubic, &
     Helm_cos, Helm_lin, Helm_mix, hkl_film, Hubb_a, Hubb_abs, Hubb_d, Hubb_diag_abs, Hubbard, Kern_fast, korigimp, &
     Level_val_abs, Level_val_exc, lmaxfree, lmoins1, lplus1, &
     M1M1, Magnetic, Matper, Moment_conservation, Moy_cluster, Moy_loc, Moyenne, &
     Muffintin, No_DFT, No_interference, No_solsing, Noncentre, Nonexc_g, Nonexc, Nonsph, Normaltau, Not_centered, NRIXS, &
     Numerov, Octupole, Old_zero, One_run, One_run_L, One_SCF, Only_rixs, Operation_mode_used, Optic, Optic_xanes, Overad, &
     PointGroup_Auto, Polarise, Powder, Proto_all, Quadrupole, Recop, Relativiste, Renorm, Renorm_SCF, RIXS, RIXS_core, &
     RIXS_db_conv, RIXS_fast, RPALF, Rydberg, SCF, SCF_bulk, SCF_c, SCF_elecabs, &
  ...
  real(kind=db), dimension(:), allocatable:: Chg_coeur, cgrad, clapl, dExc_ex_nex, dista, distai, dvc_ex_nex, &
     ...
     r_cluster, r_core, rato_abs, rsato_abs, rhons, rs, rvol, sec_atom, Taux_eq, Taux_ipr, Vc, Vc_abs_i, Vh, Vhns
  real(kind=db), dimension(:,:), allocatable:: coef_g, Axe_atom_clu, Chargat_init, chargat_init_bulk, Chargat_self, &
     ...
     posi, posi_self, Psi_core, rho, rhoato_abs,rsato, V_abs_i, Vc_cluster, Vcato_abs, Vcato_init, Vcato_init_bulk, Vr, Vxc, &
     Vxc_a, Vec, xyz, Ylmso
  real(kind=db), dimension(:,:,:), allocatable:: gradvr, Int_statedens, rho_chg, rho_chg_bulk, Rho_cluster, rho_self_t, rhoato, &
     rhoato_init, rhoato_init_bulk, rot_atom, Vcato, Vcato_bulk, Vecdafse, Vecdafss, Vra, Vr_cluster, Vrato_e, Vxcato_abs, &
     Vxc_cluster, Ylmato
  ...
  real(kind=db), dimension(:,:,:,:,:,:), allocatable:: Statedens, Statedens_i
...
! In that case, there is an energy dependant truncature factor calculated in coabs/convolution and not in prepdafs.
  ...
  n_abs_bulk = 0
  do multi_run = 1,n_multi_run
    if( iabsm(multi_run) > ngroup - n_atom_bulk ) n_abs_bulk = n_abs_bulk + 1
  end do
...
! The One-run option, when used, is not used in the bulk.
    if( Bulk_step ) then
      One_run_L = .false.
    else
      One_run_L = One_run
...
! Used just for the atom position
      iabsfirst = igr_dop
    elseif( One_run_L ) then
      if( Abs_in_Bulk ) then
        iabsfirst = iabsm( n_multi_run - n_abs_bulk )
      else
        iabsfirst = iabsm(1)
      endif
    else
    ...
    First_run = .not. One_run_L &
                .or. ( One_run_L .and. ( ( multi_run_e == 1 .and. .not. Abs_in_bulk ) .or. &
                        ( multi_run == n_multi_run - n_abs_bulk .and. Abs_in_bulk ) ) )

    Second_run = multi_run_e > n_abs_bulk + 1 .and. One_run_L
    Second_SCF = multi_run_e > n_abs_bulk + 1 .and. One_SCF
...
! Maximum number of atomic orbitals
    ...
    call init_run(Bulk_step,cdil,Chargat,Charge_free,Chargm,Chargm_SCF,Clementi,Com,Doping,Ecrantage,Flapw,Force_ecr,Hubb, &
      iabsorbeur,iabsorig(multi_run),icheck(4),icom,igreq,iprabs,iprabs_nonexc,itabs,itdil,itype,itype_dop,itypepr,jseuil,lcoeur, &
      ldil,lecrantage,Lseuil,lvval,mpinodee0,mpirank0,n_atom_bulk,n_atom_proto,n_multi_run,n_orbexc,nbseuil,ncoeur,necrantage, &
      ...
      Rchimp,Relativiste,rho_coeur,rhoit,Rlapw,Rmt,Rmtimp,Self_nonexc,V_hubbard,Wien_file(8))
    ...
    if( Taux ) then
      Taux_eq(1:natomsym) = Taux_oc( abs( igreq(iprabs_nonexc,1) ) )
    else
...
! Just useful for iaabsi
    ...
    if( Bulk_step ) then
      ...
      Not_centered = Noncentre
    endif

    if( ( multi_run_e == 1 .and. .not. Abs_in_bulk ) .or. First_run ) then
...
! of the cluster.
 ...
 !                    .or. ( natome <= n_atom_proto_bulk + 1 .and. Bulk_step ) ) .and. .not. Flapw
    ...
    if( .not. Extract ) then
      ...
      if( .not. Bulk_step .and. .not. Full_atom .and. Self_nonexc .and. Self_cons_c ) then
...
! Preambule coherence:
      ...
      Recop = .false.
      if( FDM_comp .and. .not. ( Green .or. Cluster_approach ) ) then
        n_RI = 2
      else
        n_RI = 1
      endif
...
! Boucle coherence
      boucle_coh: do i_self = 1,nself_c
        ...
        if( Cluster_approach ) icheck(:) = icheck(:) - 2

        call potsup(alfpot,Axe_atom_gr,Bulk_atom_done,Cal_xanes,Chargat,Chargat_init, &
          Chargat_self,Delta_helm,dExc_ex_nex,drho_ex_nex,dv_ex_nex,dvc_ex_nex,Exc_abs_i,Excato,Full_atom,Helm_cos,Helm_lin, &
          Helm_mix,hybrid,i_self,ia_eq_inv,ia_eq_inv_self,iaabs,iaproto,iaprotoi,iapot,icheck,igreq,igroup,iprabs_nonexc, &
          ipr1,itab,itypei,itypep,itypepr,Lmax_pot,lvval,Magnetic,mpirank0,n_atom_0,n_atom_0_self,n_atom_ind, &
...
! Numerical problem around 0.
  ...
  ! For SCF, limited to 1 for H and He
            ...
            nab_coop = 0
            allocate( phiato(nphiato1,nlmagm,nspinp,nspino,n_RI,natome,nphiato7) )
            allocate( Taull(nlmagm,nspinp,nlmagm,nspinp,natome) )
            allocate( Tau_RI(nlmagm,nspinp,nlmagm,nspinp,n_RI,natome) )
...
! Loop over non equivalent atoms
            ...
            do iapr = n1,n_atom_ind
              ...
              n_atom_0,n_atom_ind,n_atom_proto,n_RI,natome,nbord,nbtm,nbtm_fdm,neqm,ngroup_m,nlm_pot, &
...
! igrph: index of representations
                do igrph = 1,ngrph
                  ...
                  if( Green ) then
                    call msm(Axe_Atom_grn,Cal_xanes,Dist_coop,Ecinetic,Eimag(ie),Full_atom, &
                    ...
                    call mat(Adimp,Ampl_dft,Atom_axe,Axe_Atom_grn,Base_hexa,Basereel,Cal_xanes,cgrad, &
                      clapl,Dist_coop,distai,E_comp,Ecinetic_out,Eclie_out,Eimag(ie),Eneg,Enervide, &
                      FDM_comp,Full_atom,gradvr,ia_coop,iaabsi,iaprotoi,iato,ibord,ich,ie,igroupi,igrph,irep_util, &
                      isbord,iso,ispin,isrt,ivois,isvois,karact,lato,Lmaxa,Lmaxso,lso,mato,mpinodes,mpirank0,mso,n_atom_0, &
                      n_atom_coop,n_atom_ind,n_atom_proto,n_RI,nab_coop,natome,nbm,nbord,nbordf,nbtm,ngroup_m,ngrph, &
                      nim,nicm,nlm_rixs,nlmagm,nlmmax,nlmomax,nlmsa,nlmsam,nlmso,nphiato1,nphiato7,npoint,npr,npsom,nsm, &
                      ...
                      Tau_RI,Taull,Test_reg,Time_fill,Time_tria,V0bdc_out,Vr,xyz,Ylm_comp,Ylmato,Ylmso)
                  endif
            ...
            call Cal_dens(Cal_xanes,Density_comp,drho_self,Ecinetic,Eimag(ie),Energ(ie),Enervide,Full_atom, &
...
! Exchange of tables for SCF
        if( mpinodes0 > 1 ) then
          ...
          n = ( 1 + nrm_self ) * nspin * m * nlm_pot
            ...
            call MPI_Bcast(rho_self_s,n,MPI_REAL8,0,MPI_COMM_WORLD,mpierr)
            call MPI_Bcast(Chargat_self,m2,MPI_REAL8,0, MPI_COMM_WORLD,mpierr)
            call MPI_Bcast(Chargat_self_s,m2,MPI_REAL8,0, MPI_COMM_WORLD,mpierr)
            call MPI_Bcast(pop_orb_val,m2,MPI_REAL8,0, MPI_COMM_WORLD,mpierr)
...
! For H2, N2, O2, F2, Cl2 and the rare gaz, the tabulated Edge values are versus vacuum. We shift it to be versus Fermi
    if( .not. Optic ) then
      ...
      if( Z == 1 .or. Z == 2 .or. Z == 7 .or. Z == 8 .or. Z == 9 .or. Z == 10 .or. Z == 17 .or. Z == 18 .or. Z == 36 &
               .or. Z == 54 .or. Z == 86 ) Eseuil(:) = Eseuil(:) + E_Fermi
    endif
...
! Calculation of useful representations, that is probed by the photoelectron
    if( .not. ( Density .or. Optic .or. NRIXS .or. COOP .or. RIXS .or. Cluster_approach ) ) then
...
! Elaboration of the FDM grid of point. Even in Green, one defines a grid of point to calculate the average interstitial potential
      ...
      nlm_pot_g = 0
      nlm_pot_r = 0
      if( Cluster_Approach ) then
        if( Lmax_pot_c < 0 ) then
          call clmax(Energ_s(nenerg_s),Rsort,-1,Lmax_cluster,-1,.true.)
        else
          Lmax_cluster = Lmax_pot_c
        endif
        do i = 1,2
          if( i == 2 ) nlm_pot_g = n
          allocate( Lm_cluster(nlm_pot_g,2) )
          call Lm_cluster_cal(i,icheck(11),iopsymr,Lm_cluster,Lmax_cluster,n,nlm_pot_g)
          if( i == 1 ) deallocate( Lm_cluster )
        end do
        if( Ylm_comp ) then
          nlm_pot_cp = nlm_pot_g
        else
          nlm_pot_r = nlm_pot_g
        endif
        do i_cycle = 1,2
          allocate( r_cluster(nr_cluster) )
          if( Numerov ) then
            call Cluster_radial_grid_Numerov(i_cycle,icheck(11),natome,nr,nr_cluster,posi,r_cluster,Rsort)
          else
!            call Cluster_radial_grid_Numerov(i_cycle,icheck(11),natome,nr,nr_cluster,posi,r_cluster,Rsort)
            call Cluster_radial_grid(i_cycle,icheck(11),itypei,natome,nr,nr_cluster,nrato,nrm,ntype,posi,r_cluster,Rato,Rsort)
          endif
          if( i_cycle == 1 ) then
            deallocate( r_cluster )
            nr_cluster = nr
          endif
        end do
        do i_cycle = 1,2
          allocate( r_core(nr_core) )
          allocate( Psi_core(nr_core,nbseuil) )
          call Core_Psi_radial_grid(i_cycle,icheck(11),nbseuil,nr,nr_cluster,nr_core,nrato_abs,nrm,Psi_core,Psii,r_cluster,r_core, &
                                Rato_abs)
          if( i_cycle == 1 ) then
            deallocate( Psi_core, r_core )
            nr_core = nr
          endif
        end do
      ...
      allocate( Vc_cluster(nr_cluster,nlm_pot_r) )
      allocate( Vc_cluster_cp(nr_cluster,nlm_pot_cp) )
      allocate( Vr_cluster(nr_cluster,nspin,nlm_pot_r) )
      allocate( Vxc_cluster(nr_cluster,nspin,nlm_pot_r) )
      allocate( Vr_cluster_cp(nr_cluster,nspin,nlm_pot_cp) )
      allocate( Vxc_cluster_cp(nr_cluster,nspin,nlm_pot_cp) )
      allocate( Rho_cluster(nr_cluster,nspin,nlm_pot_r) )
      allocate( Rho_cluster_cp(nr_cluster,nspin,nlm_pot_cp) )

      if( Cluster_Approach ) &
          call PotSup_SphereHarm(alfpot,Energ_s(nenerg_s),iaproto,icheck,igrpt_nomag,iopsymr,itypep,itypepr,Lm_cluster, &
...
! Potential calculation
        ...
        if( cluster_approach ) icheck(13) = icheck(13) - 2
        call Potential_comp(Bulk_atom_done,Cal_xanes,distai(natome),dV0bdcF,Full_atom, &
          Green,i_range,i_self,iaabs,iapot,iaproto,iaprotoi,icheck(13),imoy,imoy_out,iopsymr,ipr1,isrt,itypepr,korigimp,Magnetic, &
          Moy_loc,mpirank0,n_atom_0,n_atom_ind,n_atom_proto,n_atom_proto_bulk,natome,natomp,nim,nlm_pot,npoint,npsom,nptmoy, &
          nptmoy_out,nrato,nrm,nsortf,nspin,nstm,ntype,numat,poidsov,poidsov_out,pos, &
        ...
        if( cluster_approach ) icheck(13) = icheck(13) + 2
         ...
         V0bdcF_out(1:nspin) =  V0bdcF(1:nspin)
...
! Determination of Lmax
      ...
      nlmmax = (Lmax_max + 1 )**2
      if( Tddft .or. Optic .or. RIXS_db_conv ) then
...
!  nlmmax = (Lmax_max + 1 )**2   max dimension
        ...
        if( Tddft_xanes .or. Optic_xanes .or. RIXS_db_conv ) then
          rof0(:,:,:,:,:) = (0._db,0._db)
...
! One makes the storage only when mpinodee > 1, otherwise the writing is a temporary file
      if( One_run_L .and. First_run .and. i_range == 1 ) &
        allocate( Taull_stk(nlm_probe,nspinp,nlm_probe,nspinp,n_multi_run-n_abs_bulk-1,nge) )
...
! Calculation of potential in the excited state
          ...
          elseif( Cluster_approach ) then
            call Potex_cluster(alfpot,Enervide,icheck(16),Lm_cluster,nlm_pot_cp,nlm_pot_g,nlm_pot_r,nr_cluster,nspin, &
          ...
          do ispin = 1,nspin
            ...
            if( .not. Cluster_approach ) V0bdc_out(ispin) = V0bdc(ispin)
...
! Numerical problem around zero
          ...
          Tau_RI(:,:,:,:,:,:) = (0._db,0._db)
          Tau_coop(:,:,:,:,:,:) = (0._db,0._db)
          allocate( Rad_ioRIoi(nlm_probe,nlm_probe,nspinp,ip_max-ip0+1,ip_max-ip0+1,nbseuil) )
...
! Loop over non-equivalent atoms
            ...
            do iapr = n1,n_atom_ind
              ...
              call Sphere(Axe_atom_grn,Ecinetic,Eimag(ie),Energ(ie),Enervide,Full_atom, &
                ...
                m_hubb,n_atom_0,n_atom_ind,n_atom_proto,n_RI,natome,nbord,nbtm,nbtm_fdm,neqm,ngroup_m, &
...
! igrph: loop over representations
          ...
          if( Cluster_approach ) then
            call Radial_FDM_comp_cluster(E_comp,Ecinetic_out,Eimag(ie),Energ(ie),Enervide,Eseuil,Hubb_abs, &
              Hubb_diag_abs,icheck(19),ip_max,ip0,iso,Lm_cluster,Lmaxso,Lso,m_hubb,mso,nbseuil,ngrph, &
              ...
              V_hubb_abs,V0bdc_out,Vr_cluster,Vr_cluster_cp,Ylm_comp)

          elseif( .not. ( Second_run .or. Extract ) ) then
          ...
          if( One_run_L .and. .not. Extract ) &
            call Data_one_run(First_run,iabsm,icheck(19),igreq,index_e,igroupi,ipr1,Lmax_probe,Lmaxa, &
          ...
          if( Extract .and. RIXS ) call Ampl_reading(Ampl_dft,icheck(19),ie,Lmaxso,mpinodee,mpirank0,nenerg,nlm_probe, &
          ...
          if( Optic_xanes .or. Tddft_xanes .or. RIXS_db_conv ) then
...
! For the atomic part, all signal is in the singular solution
              ...
              if( FDM_comp ) then
                do Lm1 = 1,nlm_probe
                  do Lm2 = 1,nlm_probe
                    Tau_RI_abs(Lm1,:,Lm2,:,:) = Tau_RI(Lm1,:,Lm2,:,:,iaabsi)
                  end do
                end do
              endif
              ...
              if( mpirank_in_mumps_group == 0 ) then
                if( NRIXS ) &
                  call S_nrixs_cal(coef_g,Core_resolved,Ecinetic, &
                    Eimag(ie),Energ(ie),Enervide_t,Eseuil,FDM_comp,Final_tddft,Full_potential,Green,Green_i,Hubb_abs, &
                    Hubb_diag_abs,icheck(20),l0_nrixs,Lmax_nrixs,is_g,Lmax_probe,Lmax_pot,lmoins1,lplus1,Lseuil,m_g,m_hubb, &
                    mpinodee,mpirank,n_Ec,n_RI,n_V,nbseuil,ns_dipmag,ndim2, &
                    ...
                    S_nrixs,S_nrixs_m,Solsing,Solsing_only,Spinorbite,Tau_RI_abs,Taull_abs, &
                    V_hubb_abs,V_intmax,V0bdc,Vra,Ylm_comp)

                if( Cluster_approach ) then
                  call Tenseur_car_cluster(coef_g,Core_resolved,Ecinetic,Eimag(ie), &
                    Energ(ie),Enervide_t,Eseuil,FDM_comp,Final_optic,Final_tddft,Full_potential,Green,Green_i,Hubb_abs, &
                    Hubb_diag_abs, &
                    icheck(20),ie,ip_max,ip00,is_g,Lmax_probe,Lmax_pot,ldip,lmoins1,loct,lplus1,lqua,Lseuil,m_g,m_hubb, &
                    mpinodee,mpirank,mpirank0,msymdd,msymddi,msymdq,msymdqi,msymdo,msymdoi,msymoo,msymooi,msymqq,msymqqi, &
                    Multipole, &
                    n_Ec,n_oo,n_rel,n_V,nbseuil,ns_dipmag,ndim2,nenerg_tddft,ninit1,ninit,ninitr,ninitlv,nlm_pot,nlm_probe, &
                    nlm_p_fp,nlmamax,nrato_abs,nrm,nspin,nspino,nspinp,numat(itabs),psii,rato_abs,Rad_ioRIoi,Relativiste,Renorm, &
                    Rmtg_abs,Rmtsd_abs,rof0,rot_atom_abs,Rot_int, &
                  ...
                  call Tenseur_car(coef_g,Core_resolved,Ecinetic,Eimag(ie), &
                    Energ(ie),Enervide_t,Eseuil,FDM_comp,Final_optic,Final_tddft,Full_potential,Green,Green_i,Hubb_abs, &
                    ...
                    Multipole,n_Ec,n_oo,n_rel,n_RI,n_V,nbseuil,ns_dipmag,ndim2,nenerg_tddft,ninit1,ninit,ninitr,ninitlv,nlm_pot, &
                    ...
                    secmm_m,secoo,secoo_m,secqq,secqq_m,Solsing,Solsing_only,Spinorbite,Tau_RI_abs,Taull_abs,Tddft,V_hubb_abs, &
                    V_intmax,V0bdc,Vra,Ylm_comp)
                endif
              endif
            ...
            deallocate( taull_abs, Tau_RI_abs )
...
!                Spinorbite,State_all_out,Taull,Tau_comp,V_hubb,V_hubb_abs,V_intmax,V0bdc,Vrato,Ylm_comp)
          ...
          deallocate( Rad_ioRIoi, Tau_coop, Tau_RI, Taull )
        ...
        if( mpinodee > 1 ) then
          ...
          if( Tddft_xanes .or. Optic_xanes .or. RIXS_db_conv ) &
        ...
        do ie_computer = 0,mpinodee-1
          ...
          if( mpirank0 == 0 ) then
            ...
            if( icheck(19) > 1 .and. .not. Cluster_approach ) &
            ...
            if( icheck(19) > 0 ) &
              call Tau_writing(Eimag(ie),Energ(ie),iprabs,ie_computer,itypepr,lmax_dft,Lmax_probe,mpinodee,n_atom_proto, &
                               nlmmax,nspinp,ntype,numat,Taull_dft)
            ...
            if( .not. Optic .and. .not. Only_RIXS ) then
              ...
              if( Abs_in_Bulk_roughness .and. DAFS ) then
              ...
              if( Abs_in_Bulk_roughness .and. DAFS ) multi_0 = multi_0 - 1
   ...
   ! One keeps the bulk atom potentials
      ...
      deallocate( Rho_cluster, Rho_cluster_cp, Vc_cluster, Vc_cluster_cp, Vr_cluster, Vr_cluster_cp,Vxc_cluster, Vxc_cluster_cp  )
      if( Cluster_Approach ) deallocate( Lm_cluster, r_cluster )
      ...
      if( .not. ( ( One_run_L .and. ( ( .not. Abs_in_bulk .and. multi_run /= n_multi_run ) &
                                 .or. ( Abs_in_bulk .and. multi_run /= 1 ) ) .or. Extract ) ) ) &
        deallocate( Excato, rs, rhoato, rsato, Vcato, Vh, Vhns, Vxc, Vxcato )

    end do boucle_i_range

    if( One_run_L .and. ( ( .not. Abs_in_bulk .and. multi_run == n_multi_run ) .or. ( Abs_in_bulk .and. multi_run == 1 ) ) ) &
...
!----------------------------------------------------------------------------------------------------------------------------------------------
    ...
    if( RIXS ) then
      ...
      if( RIXS_db_conv ) then

        call RIXS_DoubleConv(Coef_g,Core_resolved,dV0bdcF,E_cut,E_cut_imp,E_Fermi,E_cut_man,Eclie,Eneg, &
          Energ_loss_rixs,Energ_out_rixs,Energ_rixs,Energ_s,Epsii,Epsii_ref,Epsii_ref_man,Eseuil,Estart,File_name_rixs(multi_run), &
          FDM_comp,Full_potential, &
          Gamma_hole,Gamma_hole_man,Green,Green_int,Hubb_abs,Hubb_diag_abs,iabsorig(multi_run), &
          icheck(29),isymeq,ip0,ip_max,is_g,jseuil,ldip,lmoins1,loct,lplus1,lqua,lseuil,m_g,m_hubb, &
          ...
          nrato_abs,nrm,ns_dipmag,nspin,nspino,nspinp,numat_abs,Orthmati,Powder,psii,q_rixs, &
          rato_abs,Relativiste,Renorm,Rmtg_abs,Rmtsd_abs,Rot_atom_abs,Rot_int,Shift_core,Solsing,Solsing_only, &
          Spinorbite,Step_loss,Taull_tdd,Theta_rixs,V_intmax,V_hubb,Vcato_abs,Vhbdc,VxcbdcF,Vxcato_abs,Ylm_comp)

        if( All_site_rixs .and. n_multi_run > 1 .and. mpirank0 == 0 ) then
          allocate( File_name(1) )
          File_name(1) = File_name_rixs(multi_run)
          call Write_int_db_rixs(Analyzer,File_name(1),1,Sum_rixs)
          deallocate( File_name )
        endif

        if( multi_run == n_multi_run .and. mpirank0 == 0 ) then
          if( mpirank0 == 0 ) call Write_int_db_rixs(Analyzer,File_name_rixs,n_multi_run,Sum_rixs)
        ...
        call main_RIXS(Ampl_abs,Circular,Coef_g,Core_resolved,dV0bdcF,E_cut,E_cut_imp,E_Fermi,E_cut_man,Eclie,Eneg, &
          Energ_loss_rixs,Energ_out_rixs,Energ_rixs,Energ_s,Epsii,Epsii_ref,Epsii_ref_man,Eseuil,Estart, &
          File_name_rixs(multi_run),FDM_comp,Full_potential,Gamma_hole,Gamma_hole_man,Hubb_abs,Hubb_diag_abs, &
          iabsorig(multi_run),icheck(29),igreq,iprabs_nonexc,isymeq,ip0,ip_max,is_g,jseuil,lmoins1,lplus1,Lseuil,m_g,m_hubb, &
          Moment_conservation,multi_run, &
          Multipole,n_atom_proto,n_atom_uc,n_multi_run,n_oo,n_q_rixs,n_rel,n_shift_core,n_theta_rixs,natomsym,nbseuil, &
          nenerg_in_rixs,nenerg_loss_rixs,nenerg_out_rixs,nenerg_s,neqm,ngamh,ninit1,ninit,ninitr,nlm_pot,nlm_probe,nlmomax, &
          nomfich,nrato_abs,nrm,ns_dipmag,nseuil,nspin,nspino,nspinp,numat_abs,Orthmati,Posn,Powder, &
          psii,q_rixs,rato_abs,Relativiste,Renorm,RIXS_fast,Rmtg_abs,Rmtsd_abs,Rot_atom_abs,Rot_int,Shift_core,Spin_channel, &
          Spinorbite,Step_loss,Theta_rixs,V_intmax,V_hubb,Vcato_abs,Vhbdc,VxcbdcF,Vxcato_abs,Ylm_comp)

        if( All_site_rixs .and. n_multi_run > 1 .and. mpirank0 == 0 ) then
          allocate( File_name(1) )
          File_name(1) = File_name_rixs(multi_run)
          call Write_Int_rixs(Ampl_rixs,Analyzer,File_name(1),File_name_int,1,No_interference,RIXS_core,Sum_rixs,Write_modQ)
          deallocate( File_name )
        endif

        if( multi_run == n_multi_run ) then
          if( mpirank0 == 0 ) call Write_Int_rixs(Ampl_rixs,Analyzer,File_name_rixs,File_name_int,n_multi_run,No_interference, &
...
!      if( RIXS .and. multi_run == n_multi_run ) call Convert_to_xyz(File_name_int)
    ...
    if( Tddft .or. Optic ) then
      ...
      if( Optic ) then
        ...
        if( .not. No_DFT ) call main_optic(Abs_U_iso,angxyz,Allsite,axyz,Bragg_abs,Cartesian_tensor, &
        ...
        if( Tddft ) call main_tddft_optic(Abs_U_iso,alfpot,angxyz,Allsite,Atomic_scr,axyz,Bragg_abs, &
          coef_g,Cartesian_tensor,Core_resolved,Dafs,Dafs_bio,Dipmag, &
      ...
      elseif( Tddft ) then
        ...
        call main_tddft(Abs_in_Bulk_roughness,Abs_U_iso,alfpot,All_nrixs,angxyz,Allsite,Atomic_scr,axyz,Bragg_abs, &
          Bragg_rgh_bulk_abs,Bulk_step,coef_g,Cartesian_tensor,Core_resolved,Dafs,Dafs_bio,Delta_edge,Delta_Eseuil, &
    ...
    if( .not. Extract ) then
      if( Bulk_step .and. .not. Full_atom .and. Self_nonexc .and. self_cons_c ) then
   ! One keeps the bulk atom potentials
    ...
    if( ( .not. One_run_L .or. multi_run_e == n_multi_run ) .and. .not. Extract ) then
      deallocate( xyz )
