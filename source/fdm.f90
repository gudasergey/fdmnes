...
subroutine fdm(Ang_borm,Bormann,Check_extract,Comt,Convolution_cal,Delta_edge,E_cut_imp,E_cut_man,Ecent,Elarg,Estart,Fit_cal, &
  ...
  logical:: Absauto, Abs_in_bulk, All_nrixs, Allsite, ATA, Atom_nonsph, Atom_occ_hubb, Atomic_scr, Axe_loc, &
     ...
     Polarise, Quadmag, Quadrupole, Readfast, Relativiste, Renorm, RPALF, Rydberg, Scan_a, Self_abs, &
...
subroutine Site_calculation(adimp_e,alfpot,All_nrixs,Allsite,Ang_rotsup,Angle_mode,Angle_or,Angpoldafs,Angxyz,Angxyz_bulk, &
  ...
  logical:: Abs_in_bulk, Abs_in_Bulk_roughness, Absorbeur, All_nrixs, Allsite, ATA, Bulk_atom_done, Atom_comp_cal, Atom_nonsph, &
     ...
     SCF_mag_free, Second_run, Second_SCF, Self_abs, Self_cons, Self_cons_bulk, Self_cons_c, Self_nonexc, &
  ...
  real(kind=db), dimension(:,:), allocatable:: coef_g, Axe_atom_clu, Chargat_init, chargat_init_bulk, Chargat_self, &
     ...
     posi, posi_self, rho, rhoato_abs,rsato, V_abs_i, Vcato_abs, Vcato_init, Vcato_init_bulk, Vr, Vxc, vec, xyz, xyz_so
...
! Making of the FDM grid of points. Even with Green, it is used for the calculation of the interstitial potential
      ...
      allocate( xyz_so(4,nsort) )
      if( .not. Green ) Call Selec_xyz_so(iopsymr,isrt,npsom,nsort,xyz,xyz_so)
...
! Calcul des Ylm sur les points du maillage
          ...
          if( .not. Green )  then
            call Ylmpt(iaprotoi,ibord,icheck(15),iopsymr,lmaxat,n_atom_proto,natome, &
                ...
                do igrph = 1,ngrph
                  ...
                  if( Green ) then
                    ...
                    call mat(Adimp,Atom_axe,Axe_Atom_grn,Base_hexa,Basereel,Cal_xanes,cgrad, &
                      ...
                      Time_fill,Time_tria,V0bdc_out,Vr,xyz,xyz_so,Ylm_comp,Ylmato)
                  endif
...
! Preparation of next iteration for self-consistence
   ...
   ! One keeps the bulk atom occupancies
      ...
      deallocate( Statedens, Statedens_i, V_hubb_s, Vcato, Vh, Vhns, Vr, Vrato, Vxc, Vxcato, xyz, xyz_so, Ylmato )
      if( .not. Green ) deallocate( gradvr )
...
! inside the cluster
      ...
      if( Hubb(itabs) .and. i_range == 1 .and. .not. extract ) then
        if( Full_atom ) then
          iapr = iaabsi
...
! Calculation of the table dimensions for the FDM grid of points
        ...
        if( i_range > 1 ) deallocate( clapl, cgrad, ivois, isvois, numia, rvol, xyz, xyz_so )
...
! Elaboration of the FDM grid of point. Even in Green, one defines a grid of point to calculate the average interstitial potential
      ...
      if( .not. ( Green .or. Second_run .or. Extract ) ) then
        ...
        allocate( xyz_so(4,nsort) )
        Call Selec_xyz_so(iopsymr,isrt,npsom,nsort,xyz,xyz_so)
      endif
...
!----------------------------------------------------------------------------------------------------------------------------------------------
   ...
   ! One keeps the bulk atom potentials
    ...
    if( .not. ( Green .or. Second_run .or. Extract ) ) deallocate( xyz_so )
