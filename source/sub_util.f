...
*     IF( FIRST ) THEN
         ...
         EPS = DLAMCH( 'E' )
!
! Correction Y Joly car log(zero)  yj
!         write(6,'(/A)') ' erreur ici: safmin  eps  DLAMCH   SAFMIN/EPS'
!         write(6,'(1p,4e13.5)') safmin,eps,DLAMCH( 'B' ),SAFMIN/EPS, 
!
         xxxx = max(SAFMIN/EPS, 2.00D-292)         
!
         SAFMN2 = DLAMCH( 'B' )**INT( LOG( xxxx ) /
     $            LOG( DLAMCH( 'B' ) ) / TWO )
...
!
!      write(6,'(A,1p,3e13.5)')
!     &              ' erreur_ici ',LOG( SAFMIN / EPS ), SAFMN2, SAFMX2
*        FIRST = .FALSE.
*     END IF
      IF( G.EQ.ZERO ) THEN
         CS = ONE
